...
export default function MoviesSettings() {
  ...
  const [originalConfig, setOriginalConfig] = useState<MoviesConfig | null>(null)
  ...
  // Fetch movies config
  const { data: config, isLoading, error } = useQuery<MoviesConfig>({
    queryKey: ['movies-config'],
    queryFn: () => axios.get('/api/config/movies/').then(res => res.data),
  })
  ...
  if (isLoading) {
    return (
      <>
        <PageHeader title="Movie Settings" description="Configure movie library and download settings" />
        <div className="p-6">
        ...
        <div className="p-6">
          <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-6">
            <div className="text-red-400">
              ...
              <p>Unable to load movie settings. Please try again later.</p>
            </div>
          </div>
        </div>
      </>
  ...
  if (!editableConfig) {
    return (
      <>
        <PageHeader title="Movie Settings" description="Configure movie library settings" />
        <div className="p-6">
  ...
  return (
    <>
      <PageHeader 
        ...
        description="Configure movie library settings"
          ...
          <div className="space-y-4">
            ...
            {isAddingLibrary ? (
              <div className="flex items-center space-x-2">
                <input
                  type="text"
                  placeholder="Library Name (e.g., 4K Movies)"
