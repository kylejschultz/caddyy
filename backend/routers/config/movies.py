...
@router.put("/", response_model=MoviesConfig)
async def update_movies_config(movies_config: MoviesConfig):
    """Update the entire movies configuration."""
    config = config_manager.get_config()
    config.movies = movies_config
